...
function descriptor()
    return {
        title = "Subtitle Word Translate",
        version = "1.3",
        author = "Ye Fan",
        url = "https://github.com/sulaymanyf",
        shortdesc = "Subtitle Word Translate",
        description = "Search words from the current subtitle in a Translate engine",
        capabilities = {}
    }
end
...
local css = [[
    ...
    <style>
        ...
        th {
            background-color: #219ebc; /* Green background */
...
search_engines = {
    { name = "google", url = "http://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=zh-CN&dt=t&q=%s" },
    { name = "tureng", url = "https://tureng.com/tr/turkce-ingilizce/%s" }
}

languages = {
    {key = "Chinese", value = "zh_cn"},
...
function initialize_search_languages()
    for index, search_engine in ipairs(languages) do
        gui:inject_search_language(search_engine.key, index)
    end
end
...
function search_word()
    ...
    local selected_search_engine_name = search_engines[selected_search_engine_index].name

    local selected_search_language_index = gui:get_selected_language_dropdown_index()
    vlc.msg.info(selected_search_language_index)
    local target_language = languages[selected_search_language_index].value
    vlc.msg.info(target_language)
    ...
    if selected_search_engine_name == 'google' then
        -- 替换 google_url 中的tl的值为用户选择的目标语言
        local google_url = string.format('http://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=%s&dt=t&q=%s', target_language, M.encode_uri_component(selected_word))
        vlc.msg.info(google_url)
        online_search = OnlineSearch.new(google_url, selected_word, is_remove_head)
    else
        -- 对于非谷歌搜索引擎的处理逻辑保持不变
        local selected_search_engine_url = search_engines[selected_search_engine_index].url
        online_search = OnlineSearch.new(selected_search_engine_url, selected_word, is_remove_head)
    end
    ...
    if (content) then
        ...
        if selected_search_engine_name == 'google' then
            local html = extractTranslation(content)
            gui:print_html(hyperlink .. html)
        elseif selected_search_engine_name == "tureng" then
            local tables = ExtractTablesWithClass(content,css)
            if not tables then
                gui:print_html(hyperlink .. "not found")
            else
                gui:print_html(hyperlink .. tables[1])
            end
        else
...
function ExtractTablesWithClass(html,css)
    local pattern = "<table.->(.-)</table>"
    local tables = {}
    for tableContent in html:gmatch(pattern) do
        table.insert(tables, css..tableContent.."</table></body>")
    end
    return tables
end
...
Gui.__index = Gui
    ...
    function Gui:render()
        self.dialog = vlc.dialog("Subtitle Word Translate - Merve Araz")

        self:draw_file_section()
        self:draw_translation_language_section()
    ...
    function Gui:draw_translation_language_section()
        self.dialog:add_label("<b>Translation Language</b>", 1,  self:increment_row(), 2, 1)
        self.translation_language_dropdown = self.dialog:add_dropdown(3,  self.last_row, 3, 1)
    ...
    function Gui:inject_search_language(key, index)
        self.translation_language_dropdown:add_value(key, index)
    end
    ...
    function Gui:get_selected_translation_language()
        -- 假设 self.translation_language_dropdown 是存储目标语言下拉菜单的变量
        -- getValue 方法用于获取当前选中项的值，这里假设它返回选中的语言代码，例如 "en", "fr" 等
        local selected_language_code = self.translation_language_dropdown:get_value()
        vlc.msg.info(selected_language_code)
    ...
    function Gui:get_selected_language_dropdown_index()
        return self.translation_language_dropdown:get_value()
    end
    ...
    function extractTranslation(jsonString)
        -- 尝试匹配 JSON 结构中的翻译部分
        local translation = jsonString:match('%[%[%["(.-)"')
        return translation
    end
...
function OnlineSearch:generate_html_hyperlink()
    return "<p style = 'color:#219ebc'><strong>--------------------------Result----------------------------</strong></a></p>"
end
